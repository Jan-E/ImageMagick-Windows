...
function BuildConfiguration($config)
{
  ...
  if ($config.vstudio -eq "VS2017")
  {
    if ($config.platform -eq "x64")
    {
      New-Item -Path C:\Tools\vcpkg\installed\x64-windows\include -type directory
      Copy-Item "C:\Program Files (x86)\Microsoft Visual Studio\2017\Community\VC\Tools\MSVC\14.12.25827\atlmfc\include\*.*" "C:\Tools\vcpkg\installed\x64-windows\include\"
    }
    else
    {
      New-Item -Path C:\Tools\vcpkg\installed\x86-windows\include -type directory
      Copy-Item "C:\Program Files (x86)\Microsoft Visual Studio\2017\Community\VC\Tools\MSVC\14.12.25827\atlmfc\include\*.*" "C:\Tools\vcpkg\installed\x86-windows\include\"
    }
  }
  else
  {
    ...
    {
      New-Item -Path C:\Tools\vcpkg\installed\x64-windows\include -type directory
      Copy-Item "C:\Program Files (x86)\Microsoft Visual Studio 14.0\VC\atlmfc\include\*.*" "C:\Tools\vcpkg\installed\x64-windows\include\"
    }
    else
    {
      New-Item -Path C:\Tools\vcpkg\installed\x86-windows\include -type directory
      Copy-Item "C:\Program Files (x86)\Microsoft Visual Studio 14.0\VC\atlmfc\include\*.*" "C:\Tools\vcpkg\installed\x86-windows\include\"
    }
  }
